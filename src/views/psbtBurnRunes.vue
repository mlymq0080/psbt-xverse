...
//构建燃烧符文脚本，Buffer.from([22,99] 这个意思是Pointer 22 指针 99 输出的位置在99，在加上不构建符石，就会不完整，不完整就会燃烧生成纪念碑。请自己了解符文协议
 const opReturnScript = bscript.script.compile([bscript.opcodes.OP_RETURN, bscript.opcodes.OP_13,Buffer.from([22,99])])
 console.log(opReturnScript)
  // 添加自定义脚本 备注amount为0
  tx.addOutput({
